...
class OpenWebUIClient:
    ...
    def list_knowledge_collections(self) -> List[Dict[str, Any]]:
        ...
        logger.error("Failed to list knowledge collections: all endpoint variations returned non-JSON responses")
        return []
    ...
    def create_knowledge_collection(self, name: str, description: str = '') -> Optional[str]:
        """
        ...
        for endpoint in endpoints_to_try:
            ...
            if response:
                ...
                try:
                    ...
                    logger.info(f"Created knowledge collection '{name}' with ID: {collection_id}")
                    ...
                    continue  # Try next endpoint
        logger.error("Failed to create knowledge collection: all endpoint variations failed")
        return None
    ...
    def upload_files_to_knowledge(
        ...
        if not knowledge_id:
            if create_if_missing:
                knowledge_id = self.create_knowledge_collection(knowledge_name, description)
                if not knowledge_id:
                    return {
                        'success': 0,
                        'failed': len(file_paths),
                        'total': len(file_paths),
                        'errors': [f"Failed to create knowledge collection '{knowledge_name}'"]
                    }
            else:
                return {
                    'success': 0,
        ...
        for file_path in file_paths:
            ...
            if file_id:
                ...
                if self.add_file_to_knowledge(knowledge_id, file_id):
                    ...
                    results['failed'] += 1
                    results['errors'].append(f"Failed to add {file_path} to knowledge collection")
            else:
                results['failed'] += 1
